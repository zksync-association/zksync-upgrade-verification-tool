...
describe('cli', () => {
  ...
  describe('facet-diff', () => {
    it('sends right arguments', async () => {
      ...
      const cli = buildCli(["facet-diff", "someDir", "SomeFacet", "--ethscankey=fakeKey"], fail, fakeCheck, fail)
      await cli.parseAsync()
      expect(called).toBe(true)
    })
  })
  ...
  describe('verifier-diff', () => {
    it('sends right arguments', async () => {
      ...
      const cli = buildCli(["verifier-diff", "someDir", "--ethscankey=fakeKey"], fail, fakeCheck, fail)
      await cli.parseAsync()
      expect(called).toBe(true)
    })
  })
  ...
  describe('download-diff', () => {
    it('sends right arguments', async () => {
      ...
      const cli = buildCli(["download-diff", "someDir", "targetDir", "--ethscankey=fakeKey"], fail, fail, fakeDownload)
      await cli.parseAsync()
      expect(called).toBe(true)
    })
  })
