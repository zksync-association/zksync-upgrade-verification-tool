...
describe('BlockExplorerClient', () => {
  describe('#getAbi', () => {
    describe('ok with a working api', () => {
      const subject = () => new BlockExplorerClient("", "https://block-explorer-api.sepolia.zksync.dev/api")
      ...
      it('returns error for a non existing address', async () => {
        const client = subject()
        await expect(client.getAbi("0xffffffffffffffffffffffffffffffffffffffff")).rejects.toThrow(ExternalApiError)
      })
      ...
      describe('when the abi was already requested once', () => {
        ...
        it<Ctx>('second call contract is fast', async ({ client }) => {
    ...
    describe('with a non existing api', () => {
      const subject = () => new BlockExplorerClient("", "https://block-explorer-api.sepolia.zksync.dev/does-not-exists")

      it('throws errors', async () => {
        const client = subject()
        await expect(client.getAbi("0x0000000000000000000000000000000000008002")).rejects.toThrow(ExternalApiError)
      })
    })
  })
  ...
  describe('#isVerified', () => {
    ...
    describe('when a wrong api', () => {
      ...
      it('errors', async () => {
        const client = subject()
        await expect(client.isVerified("0x0000000000000000000000000000000000008002")).rejects.toThrow()
      })
    })
  })

  describe('#getSourceCode', () => {
    const subject = () => new BlockExplorerClient("", "https://block-explorer-api.mainnet.zksync.io/api")
