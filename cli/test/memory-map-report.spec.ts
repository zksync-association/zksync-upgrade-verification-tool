...
describe("MemoryMapReport", () => {
  describe("For simple memory diff", () => {
    ...
    it("can display fixed array elements", () => {
      const memoryMap = new MemoryMap(diff, "addr", [], [], [
        new Property("listProp", hexToBigInt(hashedListSlot), "it is a list", new FixedArrayType(3, new BigNumberType()))
      ])
    ...
    it("can display fixed array elements when some are not present", () => {
      const memoryMap = new MemoryMap(diff, "addr", [], [], [
        new Property("listProp", hexToBigInt(hashedListSlot) + 1n, "it is a list", new FixedArrayType(3, new BigNumberType()))
      ])
    ...
    it("can display struct elements", () => {
      const memoryMap = new MemoryMap(diff, "addr", [], [], [
        new Property("listProp", hexToBigInt(hashedListSlot), "it is a list", new StructType([
          {
            name: "field1",
