...
export const downloadCode2 = async (
  ...
  const bufFile = await fs.readFile(path.join(upgradeDirectory, `${env.network}2`, "transactions.json"));
  ...
  for (const facet of facets) {
    await facet.oldAddress.map(async (addr): Promise<void> => {
    ...
    await facet.newAddress.map(async (addr): Promise<void> => {
      const contract = await env.l1Client().getSourceCode(addr)
  ...
  for (const elem of proxiedContracts) {
    ...
    if (maybeProposed.isSome()) {
      ...
      if (
        hexToBigInt(before) !== 0n &&
        hexToBigInt(after) !== 0n &&
        before !== after
      ) {
  ...
  const directDownloadContracts: HexEraPropName[] = [
    "verifierAddress",
    "admin",
  ]
