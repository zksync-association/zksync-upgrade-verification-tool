...
async function getMemoryPath (preCalculatedPath: Option<string>, env: EnvBuilder, state: ZkSyncEraState, changes: UpgradeChanges): Promise<MemoryDiffRaw> {
  return preCalculatedPath
    .map(path => env.fs().readFile(path)
      .then(buf => JSON.parse(buf.toString()))
      .then(json => memoryDiffParser.parse(json))
    ).unwrapOrElse(() => {
      return env.rpcL1().debugTraceCall(
...
export async function storageChangeCommand(env: EnvBuilder, dir: string, preCalculatedPath: Option<string>): Promise<void> {
  ...
  const memoryMap = new MemoryMap(rawMap, state.addr, [...selectors], ["0x10113bb3a8e64f8ed67003126adc8ce74c34610c"]);
