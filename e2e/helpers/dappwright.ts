import { type BrowserContext, Page, test as baseTest } from "@playwright/test";
...
export class RoleSwitcher {
  ...
  async council(page: Page, councilNumber = 0): Promise<void> {
    ...
    if (councilNumber >= councilIndexes.length) {
      throw new Error(`There is only ${councilIndexes.length} security council members. Received: ${councilNumber}`)
    }
  ...
  async guardian(page: Page, guardianNumber = 0): Promise<void> {
    ...
    if (guardianNumber >= guardianIndexes.length) {
      throw new Error(`There is only ${guardianIndexes.length} security council members. Received: ${guardianIndexes}`)
    }
  ...
  context: async ({}, use) => {
    if (!sharedBrowserContext) {
      ...
      const [wallet, _page, browserContext] = await dappwright.bootstrap("", {
        ...
        seed: process.env.MNEMONIC || "draw drastic exercise toilet stove bone grit clutch any stand phone ten",
        headless: false,
      });
