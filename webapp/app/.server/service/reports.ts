...
import { DIAMOND_ADDRS, Network, ObjectStorageChangeReport } from "validate-cli/src/index";
...
export async function checkReport(_reportId: string): Promise<CheckReportObj> {
  ...
  const { current, proposed, sysAddresses } = await calculateBeforeAndAfter(network, l1Explorer, l2Explorer);
...
export async function storageChangeReport(_reportId: string): Promise<FieldStorageChange[]> {
  ...
  const selectors = [
    ...new Set([...current.allSelectors(), ...proposed.allSelectors()])
  ]

  const facetAddrs = [
    ...new Set([...current.allFacetsAddrs(), ...proposed.allFacetsAddrs()])
  ]
  ...
  const report = new ObjectStorageChangeReport(
    memoryMap
  )
