...
describe("CLI Output Test Suite", () => {
  ...
  describe("Command: <check>", () => {
    ...
    describe("1699353977-boojum", () => {
      it.only("prints all the information for this upgrade", async () => {
        const {stdout} = await execAsync(
        ...
        function validateFacet(
          ...
          expect(lines[adminFacetStartIndex + 3]).toMatch(new RegExp(`Upgrade address.*${facetAddress}`))
          ...
          expect(lines[adminFacetStartIndex + 7]).toContain(`pnpm validate facet-diff reference/1699353977-boojum ${facetName}`)
        }
        ...
        expect(stdout).toMatch(
          /Recursion node level VkHash.*No changes/
        )

        expect(stdout).toMatch(
          /Recursion circuits set VksHash.*No changes/
        )
        ...
        const systemContractData = [
          ['EmptyContract', '0x0000000000000000000000000000000000000000', '0x01000007271e9710c356751295d83a25ffec94be2b4ada01ec1fa04c7cd6f2c7'],
          ['Ecrecover', '0x0000000000000000000000000000000000000001', '0x010000114daca2ff44f27d543b8ef67d885bfed09a74ba9cb25f5912dd3d739c'],
          ['SHA256', '0x0000000000000000000000000000000000000002', '0x010000178d93b2d7d6448866009892223caf018a8e8dbcf090c2b9053a285f8d'],
          ['EcAdd', '0x0000000000000000000000000000000000000006', '0x010000c5a85a372f441ac693210a18e683b530bed875fdcab2f7e101b057d433'],
          ['EcMul', '0x0000000000000000000000000000000000000007', '0x0100013759b40792c2c3d033990e992e5508263c15252eb2d9bfbba571350675'],
          ['EmptyContract', '0x0000000000000000000000000000000000008001', '0x01000007271e9710c356751295d83a25ffec94be2b4ada01ec1fa04c7cd6f2c7'],
          ['AccountCodeStorage', '0x0000000000000000000000000000000000008002', '0x0100009bc0511159b5ec703d0c56f87615964017739def4ab1ee606b8ec6458c'],
          ['NonceHolder', '0x0000000000000000000000000000000000008003', '0x0100012fa73fa922dd9fabb40d3275ce80396eff6ccf1b452c928c17d98bd470'],
          ['KnownCodesStorage', '0x0000000000000000000000000000000000008004', '0x0100008b0ca6c6f277035366e99407fbb4b01e743e80b7d24dea5a3d647b423e'],
          ['ImmutableSimulator', '0x0000000000000000000000000000000000008005', '0x01000047a3c40e3f4eb98f14967f141452ae602d8723a10975dc33960911d8c5'],
          ['ContractDeployer', '0x0000000000000000000000000000000000008006', '0x010006091341955c8f76409de00549fb00b275166b5a0d0d7b82cbd629bb4212'],
          ['L1Messenger', '0x0000000000000000000000000000000000008008', '0x01000301c943edb65f5a0b8cdd806218b8ecf25c022720fe3afe6951f202f3fa'],
          ['MsgValueSimulator', '0x0000000000000000000000000000000000008009', '0x0100006fa1591d93fcc4a25e9340ad11d0e825904cd1842b8f7255701e1aacbb'],
          ['L2EthToken', '0x000000000000000000000000000000000000800a', '0x01000139b506af2b02225838c5a33e30ace701b44b210a422eedab7dd31c28a3'],
          ['SystemContext', '0x000000000000000000000000000000000000800b', '0x0100023ba65021e4689dd1755f82108214a1f25150d439fe58c55cdb1f376436'],
          ['BootloaderUtilities', '0x000000000000000000000000000000000000800c', '0x010009759cab4fa9e6ca0784746e1df600ff523f0f90c1e94191755cab4b2ed0'],
          ['EventWriter', '0x000000000000000000000000000000000000800d', '0x01000019642d87621fdd82cf65aa9146486c9256d5f8849af9a37c78ef519339'],
          ['Compressor', '0x000000000000000000000000000000000000800e', '0x010001b72874590239af612f65d50a35975299f88de022493fe7f0a190e79496'],
          ['ComplexUpgrader', '0x000000000000000000000000000000000000800f', '0x0100005bfc0443349233459892b51e9f67e27ac828d44d9c7cba8c8285fd66bc'],
          ['Keccak256', '0x0000000000000000000000000000000000008010', '0x0100001fb52ca33668d01c230a1c3b13ede90fe2e37d77222410e9f183cb7a89'],
        ]

        for (const [name, addr, bytecodeHash] of systemContractData) {
          const systemContractLine = lines.findIndex(line => line.includes(name) && line.includes(addr))
        ...
        for (const [name, byteCodeHash] of specialContractData) {
          ...
          if (!line) { return expect.fail(`No line for ${name}`) }
