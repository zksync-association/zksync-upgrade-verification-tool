...
describe("storage-diff", () => {
  it("shows the correct data", async () => {
    const {stdout} = await execAsync("pnpm validate storage-diff reference/1699353977-boojum --precalculated=reference/realistic-memory-diff.json --noColor");
...
--------------------------`)
    ...
    expect(stdout).toContain(`[0xe58bb639]: .facetAddress: 0x230214f0224c7e0485f348a79512ad00514db1f7`)
    expect(stdout).toContain(`[0xfacd743b]: .facetAddress: 0x10113bb3a8e64f8ed67003126adc8ce74c34610c`)
    expect(stdout).toContain(`[0x4fc07d75]: .facetAddress: 0x10113bb3a8e64f8ed67003126adc8ce74c34610c`)
  })

  it("matches snapshot", async () => {
    const {stdout} = await execAsync("pnpm validate storage-diff reference/1699353977-boojum --precalculated=reference/realistic-memory-diff.json");
