...
describe("validate download-diff", () => {
  ...
  it('downloads all the files inside the specified directory ', async () => {
    ...
    expect(await fs.readdir(oldFolder, { recursive: false })).toEqual(expect.arrayContaining(expectedOldSubDirs))
    const newFolder = path.join(temp, 'new')
    const expectedNewSubDirs = ['facets', 'verifier', 'system-contracts', 'bootloader', 'defaultAA']
    expect(await fs.readdir(newFolder, { recursive: false })).toEqual(expect.arrayContaining(expectedNewSubDirs))
    const facets = ['AdminFacet', 'ExecutorFacet', 'GettersFacet', 'MailboxFacet']
    expect(await fs.readdir(path.join(newFolder, 'facets'), { recursive: false })).toEqual(expect.arrayContaining(facets))
    expect(await fs.readdir(path.join(oldFolder, 'facets'), { recursive: false })).toEqual(expect.arrayContaining(facets))
    const sysContracts = [
      "EmptyContract",
      "Ecrecover",
      "SHA256",
    ...
    expect(await fs.readdir(path.join(newFolder, 'system-contracts'), { recursive: false })).toEqual(expect.arrayContaining(sysContracts))
    expect(await fs.readdir(path.join(oldFolder, 'system-contracts'), { recursive: false })).toEqual(expect.arrayContaining(sysContracts))
  })
});
