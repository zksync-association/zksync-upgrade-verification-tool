...
import { RpcSystemContractProvider, SystemContractList } from "../reports/system-contract-providers";
...
export async function storageChangeCommand(
  ...
  const localFork = await withSpinner(() => LocalFork.create(env.rpcL1().rpcUrl(), env.network), "Forking network", env)

  const [pre, post] = await withSpinner(
    async () => {
      const diamond = await Diamond.create(DIAMOND_ADDRS[env.network], env.l1Client(), env.rpcL1())
      const stateTransitionManager = await diamond.getTransitionManager(env.rpcL1(), env.l1Client())
