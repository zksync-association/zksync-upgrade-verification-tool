...
describe("createDummyUpgrade", () => {
  it.skip("creates an upgrade that can be executed", async () => {
    ...
    const callData = await createFakeUpgrade(stateTransitionManager, rpc, { verifier: padHex("0x01", { size: 20 }), systemContracts: [] });
  ...
  it("creates an upgrade that can be executed with some system contracts", async () => {
    // const etherscanapikey = process.env.ETHERSCAN_API_KEY!;
    // const url = process.env.L1_RPC_URL!;
    const etherscanapikey = "88S18B5T9MQPPDCVMWHZC87FXTZ3BREAUI";
    const url = "https://mainnet.gateway.tenderly.co/1ooZXz5grkFD15LMjKrVC0";
    const explorer = BlockExplorerClient.forL1(etherscanapikey, "mainnet")
    const rpc = new RpcClient(url)
    const diamond = new Diamond(DIAMOND_ADDRS.mainnet)
    await diamond.init(explorer, rpc)
    ...
    const callData = await createFakeUpgrade(
